...
const Sidebar = ({ userType }) => {
  ...
  return (
    <div className="relative">
      <button
        className="p-2 bg-darkGray text-white fixed top-4 left-4 z-20 md:hidden"
      ...
      <aside
        ...
        } md:transform-none md:relative md:translate-x-0 z-30`}
      >
        ...
        <ul>
          {links.map((link) => (
            <li key={link.to} className="mb-4">
              <Link
                to={link.to}
                className="block py-2 px-4 text-darkGray hover:bg-gold hover:text-white rounded transition-colors"
